...
import CoolerPvP.API.EfeitosAPI.PackageType;
import CoolerPvP.API.EfeitosAPI.PacketType;
...
public enum Efeitos {
	...
	private static final Map<String, Efeitos> NAME_MAP = new HashMap<String, Efeitos>();
	private final String name;
	private final boolean requiresWater;
 
	// Initialize map for quick name lookup
	static {
		for (Efeitos effect : values()) {
	 ...
	 */
	private Efeitos(String name, boolean requiresWater) {
		this.name = name;
	 ...
	 */
	public static Efeitos fromName(String name) {
		for (Entry<String, Efeitos> entry : NAME_MAP.entrySet()) {
	...
	public static final class ParticleEffectPacket {
		...
		public static void initialize() throws VersionIncompatibleException {
			...
			try {
				...
				packetConstructor = EfeitosAPI.getConstructor(packetClass);
				getHandle = EfeitosAPI.getMethod("CraftPlayer", PackageType.CRAFTBUKKIT_ENTITY, "getHandle");
				playerConnection = EfeitosAPI.getField("EntityPlayer", PackageType.MINECRAFT_SERVER, false, "playerConnection");
				sendPacket = EfeitosAPI.getMethod(playerConnection.getType(), "sendPacket", PackageType.MINECRAFT_SERVER.getClass("Packet"));
		...
		public void sendTo(Location center, Player player) throws PacketInstantiationException, PacketSendingException {
			if (packet == null) {
				try {
					...
					EfeitosAPI.setValue(packet, true, "a", name);
					EfeitosAPI.setValue(packet, true, "b", (float) center.getX());
					EfeitosAPI.setValue(packet, true, "c", (float) center.getY());
					EfeitosAPI.setValue(packet, true, "d", (float) center.getZ());
					EfeitosAPI.setValue(packet, true, "e", offsetX);
					EfeitosAPI.setValue(packet, true, "f", offsetY);
					EfeitosAPI.setValue(packet, true, "g", offsetZ);
					EfeitosAPI.setValue(packet, true, "h", speed);
					EfeitosAPI.setValue(packet, true, "i", amount);
				} catch (Exception exception) {
