...
public static HashMap<String, AntiSpam> Players = new HashMap<String, AntiSpam>();
  ...
  public void AddMessage(AsyncPlayerChatEvent event)
  {
    ...
    if (IsSilenced())
    {
      ...
      this.Player.sendMessage(ChatColor.RED + "Você foi silenciado por Spam! " + SilenceTimeRemaining() + " segundos restantes.");
      this.Player.sendMessage(ChatColor.RED + "Se você continuar falando, voce sera banido!");
      this.Player.sendMessage(ChatColor.RED + "Você possui " + this.Warnings + " avisos restantes!");
      this.Warnings -= 1;
    }
    else
    {
  ...
  public void Silence(int seconds)
  {
    ...
    this.Player.sendMessage(ChatColor.RED + "Você foi silenciado por §6" + seconds + " segundos");
    this.Player.sendMessage(ChatColor.RED + "Se você continuar, voce sera banido!");
  }
